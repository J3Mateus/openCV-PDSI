...
class FilterUtils {
  ...
  static Future<Image?> applyThreshold(File file) async {
    ...
    try {
      ...
      var bytes = await file.readAsBytes();
      var result = await ImgProc.threshold(bytes, 90, 255, ImgProc.threshBinary);
  ...
  static Future<Image?> applyTwilight(File file) async {
    ...
    try {
      ...
      var bytes = await file.readAsBytes();
      var result = await ImgProc.applyColorMap(bytes, ImgProc.colorMapTwilightShifted);
      return Image.memory(result);
...
class FilterData {
  ...
  FilterData({this.name,required this.function});
}